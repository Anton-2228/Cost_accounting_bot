...
Поток: `WebApp.ready()` → `showScanQrPopup` (открывается сам на старте, кнопка
«Сканировать чек» — путь повтора после отмены, ошибки или добавленного чека) →
`closeScanQrPopup` → `POST …/preview` (initData в заголовке
`Authorization: tma <initData>`) → плашка → «Добавить» / «Отмена» →
`POST …/checks` → результат.
