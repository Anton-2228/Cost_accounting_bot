...
9. **Период закрывается ролловером, как только закончился.** Закрытый период не
    ...
    историю документа.
10. **Чтение архива ничего не создаёт — кроме текущего периода.** Прошлые
    периоды заводят ролловер и сама история. Исключение одно и намеренное:
    `GET /periods/current` создаёт строку под сегодня, если её ещё нет.

    Окно, ради которого это сделано, повторяется каждый месяц: в день
    `reset_day`, от местной полуночи до ближайшего тика ролловера,
    `get_containing` не находит ничего — предыдущий период кончается ровно
    сегодня, а `end_date` исключительна, новой же строки ещё нет. Запись это
    чинила сама (`ensure_current_period`), чтение — нет, и диалогу, которому
    границы нужны, чтобы задать вопрос о дне, доставался 404 там, где ответ
    вычислим. Данными пользователя период не является: он однозначно определён
    `reset_day` и сегодняшней датой, а `ensure` идемпотентен.
...
| `GET /periods/current` пишет | осознанно. Единственное чтение, которое создаёт строку, — см. §6.10. Альтернатива (считать границы на стороне бота по `reset_day`) завела бы вторую версию календаря, а отдавать 404 там, где ответ вычислим, значило бы ломать диалог на ровном месте. Коммит в сервисе обязателен: сессия сама не коммитит, репозиторий делает только `flush()`, и без него наружу уехал бы период с `id` несуществующей строки |
| Пометка чека | приходит от клиента полем `notes` и достаётся **каждой** позиции. Своей колонки у `checks` под неё нет: смотрят на пометку в строке листа операций, а строк у чека столько, сколько позиций, и «чья это пометка» говорит `check_id`. Поле необязательное — по той же причине, что и `added_at` ниже |
| День операций чека | приходит от клиента полем `added_at` и **перепроверяется**: период всегда текущий, под присланный день он не подбирается — иначе запись уходила бы в прошлый, возможно закрытый месяц, и правило «чек ложится в текущий» не жило бы нигде. Поле необязательное, пустое значит «сегодня»: так api выкатывается раньше бота, которого `extra="forbid"` иначе отверг бы целиком. Проверка стоит до первой записи — отказ по дню самый ожидаемый из отказов метода |
