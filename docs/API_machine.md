...
- **`records.check_id`** вместо `records.check_json`. Ключ составной и
  ...
  не ставится за ненадобностью — чек физически не удаляется никогда (см. шаг 6),
  и повиснуть ссылке не на чем;
...
- **`GET /spreadsheets/{id}/checks?period_id=`** — архив месяца. Своего периода
  ...
  **без** фильтра по `deleted_at` операций: пользователь, удаливший одну строку
  реестра, не отзывал чек, и зависимость от мягкого удаления позиции молча
  выносила бы чек из архива. Из архива чек выносит собственная метка удаления
  (шаг 6). Вместе с `?unprocessed=` фильтр несовместим — 422, а не пустой список;
...
### Шаг 6. Чек умирает вслед за последней своей операцией — сделан

Миграция `f2b70d5c8a19`. До неё чек был вечным: удаление всех операций,
вышедших из него, оставляло строку навсегда. Разобранный, он не возвращался в
очередь `/check`, занимал `external_key` — ту же бумажку нельзя было
отсканировать заново — и продолжал висеть строкой на листе-архиве месяца, хотя в
реестре от него ничего не осталось. Убрать его можно было только руками в SQL.

- **`checks.deleted_at`** — удаление чека стало **мягким**, как у операций.
  Физическое стёрло бы `raw_payload`, единственный след покупки, и порвало бы
  ссылки: операции, вышедшие из чека, тоже удаляются мягко и продолжают на него
  смотреть. У таблицы теперь один механизм удаления: `/check_del` переведён на
  него же (`BaseRepository.delete` снова не вызывается ниоткуда);
- **уникальность ключа стала частичной** — `WHERE deleted_at IS NULL`, поэтому
  индекс, а не констрейнт (частичным констрейнт быть не умеет). Живым может быть
  один экземпляр бумажки, история — любой длины. Без этого мягкое удаление
  означало бы «чек исчез отовсюду, а пересканировать его нельзя» — хуже
  прежнего. `uq_checks_id_spreadsheet_id` не тронут: он цель составного FK;
- **`RecordService.delete`** после мягкого удаления операции спрашивает
  `RecordRepository.exists_by_check`, и если живых позиций у чека не осталось —
  мягко удаляет чек **той же меткой времени, в той же транзакции** и ставит
  задачу `CHECKS` на период: лист-архив потерял строку. Ради этого в сервис
  добавлен `CheckRepository`;
- **`deleted_at IS NULL` во всех выборках чеков** и в `mark_processed`. Это не
  гигиена: без условия в `get_for_spreadsheet` `commit_check` записал бы операции
  в удалённый чек, а `save` отвечал бы 409 на бумажку, которую только что
  разрешили сканировать заново;
- **контракт наружу не изменился ни одним полем.** `CheckResponse` метки
  удаления не отдаёт — удалённые чеки не выдаются вовсе, показывать её некому.
  Бот тоже не тронут: удаление чека — следствие удаления операций, а не
  событие, о котором стоит сообщать.

Разобранный чек по-прежнему не удаляется через `/check_del` (409): вход в это
состояние один — удаление его операций. Два разных пути в одно состояние
позволили бы оставить реестр со строками из ниоткуда.

**Грабли миграции.** В `downgrade` мягко удалённые чеки удаляются физически
(старая схема их не выражает), а ссылавшиеся операции отвязываются
`check_id = NULL` — иначе внешний ключ не дал бы удалить ни одного чека. После
этих операторов обязателен `SET CONSTRAINTS ALL IMMEDIATE`:
`fk_records_check_id_checks` отложенный, его проверки копятся до конца
транзакции, а DDL на таблице с накопленными событиями триггеров PostgreSQL не
выполняет — `cannot CREATE INDEX "checks" because it has pending trigger events`.
...
- `BaseRepository.list()` не вызывается ни из одного репозитория — предназначен
  для будущих доменов. `delete()` (физическое удаление) тоже: `/check_del`
  какое-то время был его единственным пользователем, но с шага 6 удаление чека
  мягкое, как и всё остальное в системе.
