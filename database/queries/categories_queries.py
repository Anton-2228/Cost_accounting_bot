...
from database import (CashedRecordsOrm, CategoriesOrm, CategoriesTypes,
                      StatusTypes, session_factory)
...
class CategoriesOrmWrapper:
    ...
    def get_active_categories_by_spreadsheet(self, spreadsheet_id):
        with session_factory() as session:
            categories: list[CategoriesOrm] = session.scalars(
                select(CategoriesOrm).where(
                    (CategoriesOrm.spreadsheet_id == spreadsheet_id)
        ...
        with session_factory() as session:
            categories: list[CategoriesOrm] = session.scalars(
                select(CategoriesOrm).where(
                    ...
                    & (
                        (CategoriesOrm.status == StatusTypes.ACTIVE)
                        | (CategoriesOrm.status == StatusTypes.INACTIVE)
                    )
                )
            ).all()
            return categories
    ...
    def add_product_type_by_category_title(self, category_title: str, type: str):
        with session_factory() as session:
            category: CategoriesOrm = session.scalar(
                select(CategoriesOrm).filter(CategoriesOrm.title == category_title)
            )
    ...
    def synchronizeCategories(self, spreadsheet, scope, spreadsheetWrapper):
        with session_factory() as session:
            ...
            spreadsheets_categories = spreadsheetWrapper.getValues(
            ...
            if "values" in spreadsheets_categories:
                for i in range(len(copy.deepcopy(spreadsheets_categories["values"]))):
                    ...
                    if len(row) != 0:
                        spreadsheets_categories["values"][i].extend(
                            [""] * (7 - len(row))
                        )
                result = {"result": "error"}
                ...
                for z, row in enumerate(spreadsheets_categories["values"]):
                    ...
                    if row[0] != "":
                        ...
                        if row[6].strip() != "":
                            category.product_types = list(
                                set([x.lower().strip() for x in row[6].split(",")])
                            )
                        else:
                            category.product_types = []
                        self.delete_cashed_names_by_deleted_product_types(
                            category.product_types, last_product_types, session
                        )
                        categories.append(
                            [
                                row[0],
                                ...
                                row[3],
                                row[4],
                                " ".join(category.associations),
                                ", ".join(category.product_types),
                            ]
                        )
                    else:
                        if row[1] == "1":
    ...
    def delete_cashed_names_by_deleted_product_types(
        self, current_product_types, last_product_types, session
    ):
