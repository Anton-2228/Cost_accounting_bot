    ...
    async def execute(
        ...
        if cur_state == None:
            ...
            if text != "ПОДТВЕРЖДАЮ УДАЛЕНИЕ":
                ...
                user = self.postgres_wrapper.users_wrapper.get_user(
                    message.from_user.id
                )
                spreadsheet = (
                    self.postgres_wrapper.spreadsheets_wrapper.get_spreadsheet(
                        message.from_user.id
                    )
                )
                self.postgres_wrapper.users_wrapper.remove_user(user.id)
                self.postgres_wrapper.spreadsheets_wrapper.remove_spreadsheet(
                    spreadsheet.id
                )
                await state.clear()
