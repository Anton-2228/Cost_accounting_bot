...
class DeleteRecord(Command):
    async def execute(self, message: Message, state: FSMContext, command: CommandObject):
        ...
        self.postgres_wrapper.sources_wrapper.update_current_balance(delete_record.source, -delete_record.amount)

        values = []
        value_record = await sync_records_from_db_to_table(spreadsheet, self.postgres_wrapper)
