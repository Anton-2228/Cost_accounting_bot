...
from database import (CategoriesOrm, CategoriesTypes, PostgresWrapper,
                      RecordsOrm, SpreadSheetsOrm, StatusTypes)
from init import alf, daysUntilNextMonth
...
async def sync_cat_from_table_to_db(
    ...
    resultSyncCat = postgres_wrapper.categories_wrapper.synchronizeCategories(
        spreadsheet, "Categories!A2:G100000", spreadsheetWrapper
    )
...
async def sync_sour_from_table_to_db(
    ...
    resultSyncSour = postgres_wrapper.sources_wrapper.synchronizeSources(
        spreadsheet, "Bills!A2:F100000", spreadsheetWrapper
    )
...
async def sync_cat_from_db_to_table(
    ...
    categories: list[CategoriesOrm] = (
        postgres_wrapper.categories_wrapper.get_active_ans_inactive_categories_by_spreadsheet(
            spreadsheet.id
        )
    )
    values = []
...
async def sync_total_from_db_to_table(
    ...
    categories: list[CategoriesOrm] = (
        postgres_wrapper.categories_wrapper.get_active_categories_by_spreadsheet(
            spreadsheet.id
        )
    )
    ...
    for i in income_categories:
        records: list[RecordsOrm] = (
            postgres_wrapper.records_wrapper.get_records_by_current_month_by_category(
                spreadsheet.id, spreadsheet.start_date, i.id
            )
        )
