...
from database import (CategoriesOrm, CategoriesTypes, PostgresWrapper,
                      SourcesOrm, SpreadSheetsOrm)
from datafiles import ADD_RECORD_MESSAGE
from spreadsheet_wrapper import SpreadsheetWrapper


def parse_records_row(
    row, categories: list[CategoriesOrm], sources: list[SourcesOrm] = None
) -> dict:
    response = {}
...
async def add_record(
    ...
    spreadsheetWrapper: SpreadsheetWrapper,
    ...
    if category.type == CategoriesTypes.INCOME:
        ...
        record = postgres_wrapper.records_wrapper.create_record(
    ...
    records = postgres_wrapper.records_wrapper.get_records_by_current_month(
...
def create_add_message(data: dict) -> str:
    ...
    return ADD_RECORD_MESSAGE.format(
        amount=amount,
        typeCat=typeCat,
