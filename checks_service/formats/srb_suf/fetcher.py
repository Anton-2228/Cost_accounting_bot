    ...
    def _version(
        ...
        for element_id, label in labels.SPAN_FIELDS:
            ...
            if element_id in labels.MONEY_SPAN_IDS:
                value = _decimal_point(value, locale)
...
def _decimal_point(value: str, locale: str) -> str:
    """Денежная строка со страницы в канонической точечной записи.

    Каждый язык печатает сумму по своим правилам, и **оба** разделителя в них
    участвуют: сербская страница пишет «3.584,00», английская — «3,584.00».
    Значит, роль знака зависит от языка, а не от самого знака, и разбирать
    сумму, не зная, на какой странице она напечатана, нельзя вовсе.

    Прежняя версия этого не знала и просто меняла запятую на точку. На суммах
    меньше тысячи разделителя разрядов нет, и подмена сходилась случайно —
    ровно на таких чеках всё и проверялось. Первая же тысяча ломала обе версии
    в одну и ту же нечитаемую «3.584.00», а бот отвечал «суммы записаны не
    числами»: чек к тому моменту уже лежал в БД, и вернуть его было нечем,
    кроме `/check_del`.
    """
    if locale == constants.SRB_SUF_LOCALE_SR:
        # Точка — разряды, запятая — дробная часть.
        return value.replace(".", "").replace(",", ".")
    # Английская запись: запятая — разряды, точка уже на своём месте.
