...
class ChecksServiceSettings(BaseSettings):
    ...
    # ---- Внешняя расшифровка чеков (Сербия, ПУРС) ----
    #: Токена здесь нет: страница проверки чеков открыта, и платить за неё не
    #: приходится. Ограничение доступа к сервису это не отменяет — оно защищает
    #: не лимит, а чужую таблицу.
    suf_base_url: str = "https://suf.purs.gov.rs"
    #: Таймаут щедрее российского: расшифровка стоит трёх запросов подряд —
    #: страница на двух языках и отдельный запрос позиций.
    suf_timeout_seconds: float = 30.0
