...
class ReceiptNotReadyError(ReceiptFetchError):
    """Чек подлинный, но позиций у налоговой по нему ещё нет.

    Сербская страница чека проверяет подпись прямо из QR и отдаётся сразу, а
    позиции берутся из базы, куда чек попадает, только когда касса его
    отправит. Чек, отсканированный у кассы, может в этот промежуток попасть —
    и повтор через пару минут здесь как раз помогает.

    4xx, а не 5xx: это ожидаемый случай, а не сбой сервиса, и трейсбек в логе
    ему не нужен.
    """
