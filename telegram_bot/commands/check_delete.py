...
class CheckDeleteCommand(BaseCommand):
    ...
    Подтверждения нет намеренно: удаляется сырьё, которое ещё ни во что не
    превратилось, и восстановить его можно повторным сканом той же бумажки.
    Разобранный чек так не удалить — api ответит 409, потому что на него
    ссылаются операции реестра.
    """
