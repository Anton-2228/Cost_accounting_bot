...
class RecordAddCommand(BaseCommand):
    ...
    Названный день обязан лежать в текущем периоде и не быть завтрашним —
    правило «закрытый период не меняется» этим не нарушается, задним числом
    можно поправить только сегодняшний месяц, а вперёд нельзя вовсе.
