"""FSM-состояния бота.

Один `StatesGroup` на весь бот: состояний немного, и разбиение по группам
только добавило бы способов сослаться не туда.

Состояния заводятся **только** под многошаговые диалоги. Ошибка команды
состояние не ставит никогда: в старой версии ответ на неудачный `/sync`
переводил пользователя в `CORRECT_TABLE`, где все команды, кроме самого
`/sync`, отвечали «Исправьте таблицу», и выйти оттуда было нельзя — даже
удалить таблицу.
"""

from __future__ import annotations

from aiogram.fsm.state import State, StatesGroup


class States(StatesGroup):
    """Состояния многошаговых диалогов."""

    # Мастер создания таблицы: название → день сброса → часовой пояс → почта.
    CREATE_TABLE_TITLE = State()
    CREATE_TABLE_RESET_DAY = State()
    CREATE_TABLE_TIMEZONE = State()
    CREATE_TABLE_EMAIL = State()

    # Ожидание почты для выдачи доступа (`/table_email`).
    ADD_EMAIL = State()

    # Ожидание слова подтверждения перед отвязкой таблицы (`/table_unlink`).
    CONFIRM_UNLINK_TABLE = State()

    # Ожидание id операции перед её удалением (`/del`).
    CONFIRM_DELETE_RECORD = State()

    # Ожидание telegram id, чьи траты на модель показать (`/settings`).
    SETTINGS_ASK_TELEGRAM_ID = State()

    # Разбор чека: типы товаров → категории → день → пометка (`/check`).
    # Стадии разделены, а не слиты в одно сообщение: правка типа меняет то, о чём
    # спрашивать дальше, и одним вопросом пришлось бы просить пользователя
    # держать весь чек в голове.
    #
    # Третья стадия — день, которым датировать операции, и это не вернувшийся
    # вопрос «на какой счёт»: тот спрашивал, откуда деньги, и ушёл вместе со
    # счетами. Этот спрашивает, в какую колонку листа статистики лечь, и нужен
    # потому, что чек разбирают не обязательно в день покупки.
    #
    # Четвёртая — пометка, которая ляжет в `notes` каждой операции чека. Она
    # необязательна, и стадией — а не вопросом на любой из предыдущих — сделана
    # затем, что относится к чеку целиком, а не к позиции: спрошенная рядом со
    # списком позиций, она читалась бы как правка одной из них.
    CHECK_TYPES = State()
    CHECK_CATEGORIES = State()
    CHECK_DAY = State()
    CHECK_NOTES = State()
