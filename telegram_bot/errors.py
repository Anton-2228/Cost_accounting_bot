...
#: Состояние «строки в базе есть, документа в Google ещё нет». Именованной
#: константой, а не строкой внутри таблицы ответов: тот же текст печатает
#: `BaseCommand`, не дожидаясь отказа api, — и двух формулировок одного
#: состояния быть не должно.
TABLE_CREATING_MESSAGE = "Таблица ещё создаётся — пришлю ссылку, как только будет готова"
...
_CONFLICT: dict[str, str] = {
    ...
    "spreadsheet_exists": "У вас уже есть таблица. Посмотреть: /menu",
    "spreadsheet_not_ready": TABLE_CREATING_MESSAGE,
