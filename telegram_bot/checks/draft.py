...
class CheckDraft(BaseModel):
    ...
    #: Пометка, которая ляжет в `notes` каждой операции чека. Необязательна, и
    #: пустая строка значит именно «пометки нет»: `None` завёл бы второе
    #: значение с тем же смыслом, а api и без того понимает пустую строку как
    #: отсутствие. Черновик, начатый до появления стадии, читается с умолчанием
    #: и без миграции — как и `added_at` выше.
    notes: str = ""
