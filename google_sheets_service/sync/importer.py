...
class SheetImporter:
    ...
    async def import_sheet(self, state: DocumentState, target: str) -> ImportResult:
        ...
        if result.error is not None:
            # Разбор не удался: в базу не записано ничего, а отказ api уже
            # положил в уведомления — фразу на языке пользователя соберёт бот.
            # Перерисовывать нечего — лист и так соответствует базе.
            logger.info(
