...
#:
#: `CHECKS` идёт перед `OPERATIONS`: в колонке `Check` реестра стоит номер
#: чека, и к моменту его появления строка архива должна уже существовать —
#: иначе в промежутке между двумя задачами пользователь видит ссылку в пустоту.
_TARGET_ORDER = {
    "STRUCTURE": 0,
    "CATEGORIES": 1,
    "BILLS": 2,
