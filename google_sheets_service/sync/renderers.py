...
def render_operations(
    ...
    Справочник приходит вместе с удалёнными категориями: операция удалённой
    категории остаётся в реестре навсегда, и без её названия в колонке была бы
    пустота у траты, которая точно была.
    """
    category_titles = {category.id: category.title for category in categories}

    entries: list[tuple[date, int, list[dict[str, Any]]]] = [
        (record.added_at, record.id, _record_row(record, category_titles))
