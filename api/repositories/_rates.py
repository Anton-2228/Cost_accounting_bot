...
def rate_factor(
    ...
    return case(
        ...
        else_=(
            ...
            # него SQLAlchemy не догадывается, что `records` в условиях — это
            # таблица объемлющего запроса, и добавляет её в собственный FROM
            # подзапроса. Получается декартово произведение: подзапрос
            # возвращает не одну строку, а по строке на каждую операцию
