...
async def commit_check(
    ...
    День операций приходит от клиента: чек разбирают не обязательно в день
    покупки, а лист статистики раскладывает траты по дням периода. Доверия к
    присланному дню при этом нет — он перепроверяется на попадание в текущий
    период, и чужой день получает 422 `day_outside_period`.
    """
